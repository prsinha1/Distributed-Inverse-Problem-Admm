...
# 1. Compute Wi and define your original harmonic mean Qij
def make_precisions(A_dense_list):
    """
    Wi[p] = ||A_i[:, p]||_2^2
    Qij[p] = (Wi[p] * Wj[p]) / (Wi[p] + Wj[p])
    """
    eps = 1e-12
    Wi_list = []
...
def build_pixel_connected_Q_provider(base_dir="saved_operators_Incmp_Span",
    ...
    Wi_list, Qij_diag = make_precisions(A_dense_list)

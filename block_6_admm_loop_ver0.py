...
def decentralized_admm(A_dense_list, sinograms, G, Wi_list, Qij_diag_fn,
    ...
    for k in range(max_iters):
        ...
        for i in range(num_nodes):
            ...
            # --- compute and print the target tolerance for this iteration ---
            #eps0 = 1.0     # initial scale, you can tune later
            #gamma = 0.2    # exponent controlling decay
            #eps_target = eps0 / ((k + 1) ** (1.0 + gamma))
    ...
    try:
        debug_file = os.path.join(out_dir, "admm_internal_params.txt")
        with open(debug_file, "w") as f:
            f.write("===== ADMM Internal Parameters =====\n")
            f.write(f"rho = {rho}\n")
            f.write(f"lambda_tv = {lam_tv}\n")
            f.write(f"max_admm_iter = {max_admm_iter}\n")
